...
def order_tracking(request):
    ...
    if userid:
        ...
        for data in handle:   #下方白色條
            tem="Slide "+str(count)
            if count == 0:
                everyorder = {"a1": count, "b1": tem, "c1":"active"}
                ordernum.append(everyorder)
            else:
                everyorder = {"a1": count, "b1": tem, "c1":""}
                ordernum.append(everyorder)
            count+=1
        if count >= 2:
            show = "yes"
...
# def sendtoone(request, cdate, point , orderid2, tax, money):   #串接侯sir那組的碳制郎
#     userid = request.session['user']
#     requests.post('https://a4d6-2001-b011-6c03-dc34-c92f-4282-7fe2-ce6c.jp.ngrok.io/api/myapp/', data = {
#         "ORDID": orderid2,
#         "MEM": userid, #userID
#         "APPID": '5',
#         "TAX": tax,
